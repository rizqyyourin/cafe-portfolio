...
beforeAll(async () => {
  await client.execute(`CREATE TABLE cafe_settings (id TEXT PRIMARY KEY NOT NULL, cafe_name TEXT NOT NULL, tagline TEXT NOT NULL, description TEXT NOT NULL, logo_url TEXT, address TEXT NOT NULL, phone TEXT NOT NULL, whatsapp TEXT NOT NULL, email TEXT NOT NULL, instagram TEXT, tiktok TEXT, facebook TEXT, maps_url TEXT NOT NULL, maps_embed_url TEXT, opening_hours TEXT NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)`);
});
...
describe("settings repository", () => {
  ...
  it("updates the single settings row and normalizes blank optional URLs to null", async () => {
    await saveCafeSettings({ ...values, cafeName: "Kōhi House", logoUrl: "https://example.com/logo.svg", instagram: "", openingHours: { ...values.openingHours, Sunday: "Closed" } }, testDb);

    expect(await getCafeSettings(testDb)).toMatchObject({ cafeName: "Kōhi House", logoUrl: "https://example.com/logo.svg", instagram: null, openingHours: { ...values.openingHours, Sunday: "Closed" } });
  });
});
