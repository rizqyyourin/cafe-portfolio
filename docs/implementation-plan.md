...
- Reservations module: real request list/detail, status and Jakarta today/week filters, WhatsApp links, PENDING → CONFIRMED/CANCELLED and CONFIRMED → COMPLETED transitions, server authorization/validation, revalidation, loading/error/empty/pending feedback, and stale-transition guards are complete.
